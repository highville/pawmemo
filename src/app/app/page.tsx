...
export default async function HomePage({ searchParams }: HomePageProps) {
  ...
  const [realPet, realMemories] = user
    ? await Promise.all([getFirstPet(user.id), getUserMemories(user.id, 2).then((memories) => memories.map(toAppMemory))])
    : [null, []];
    ...
    <>
      ...
      {!realPet ? (
        <Card className="bg-primary-soft/70">
          ...
          <p className="mt-2 leading-7 text-outline">Add your pet once so notes, photos, tags, and future summaries all stay connected to the right companion.</p>
      ...
      <div className="flex flex-col gap-5">
        ...
        {realPet && realMemories.length === 0 ? (
          <Card className="bg-surface-soft">
            ...
            <p className="mt-2 leading-7 text-outline">Write one small note below. It can be as simple as a nap, a walk, a meal, or a funny little moment.</p>
          </Card>
        ) : null}
        {realPet ? (
          ...
          <p className="flex items-start gap-2 leading-7">
            ...
            Tiny notes add up. AI tag ideas are optional, and your saved notes stay at the center of every letter or summary.
          </p>
          </div>
        ) : null}
      </div>
